...
| `TextField` `unit`                  | a trailing `UILabel` in the field's stack. Both static labels are hidden from VoiceOver and the field speaks for them                                                                                                                                   | a trailing `TextView` — and the row gains the leading label it never drew. The label stays a node and points at the field with `labelFor`, the unit becomes its hint text |
