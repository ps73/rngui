...
| `TextArea`   | the above plus `maxLines` — grows with its content, and takes no `unit`: a suffix has no line to sit on                                           |
...
| `TextField` `unit`                  | a trailing `UILabel` in the field's stack                                                                                                         | a trailing `TextView` — and the row gains the leading label it never drew                                                                              |
| `datePickerStyle: 'compact'` `font` | pushed onto the pill's own labels — `UIDatePicker` has no font API, so a future iOS that draws the pill differently falls back to the system font | the date is an ordinary `TextView`; it has always taken the row's font                                                                                 |
